...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (galleryElement && typeof lightGallery === 'function') {
    ...
    try {
        ...
        console.error("lgZoom is not defined. Ensure it's properly imported or included.");
    }

    try {
        lgThumbnail = window.lgThumbnail;
    } catch (error) {
        console.error("lgThumbnail is not defined. Ensure it's properly imported or included.");
    }
  ...
  if (searchInput) {
    searchInput.addEventListener('input', () => {
      ...
      galleryItems.forEach(item => {
        const itemTitle = item.querySelector('.item-info h3')?.textContent.toLowerCase() || '';
        const itemYear = item.querySelector('.item-info p')?.textContent.toLowerCase() || '';
        const itemSubHtml = item.getAttribute('data-sub-html')?.toLowerCase() || '';
        
        if (itemTitle.includes(searchTerm) || itemYear.includes(searchTerm) || itemSubHtml.includes(searchTerm)) {
          item.style.display = 'block';
        } else {
          item.style.display = 'none';
        }
  ...
  if (submissionForm) {
    submissionForm.addEventListener('submit', (e) => {
      ...
      alert('Thank you for your submission! It will be reviewed and added to the gallery soon.');
  ...
  const elementsToAnimate = document.querySelectorAll('.gallery-item, .fan-item');
